...
module Immutable
  ...
  class Vector
    ...
    # Return a new `Vector` built by subtracting the `other` vector from this one. The new vector
    # will contain the elements from this one that are not included in the `other`. `other`
    # can be any object which is convertible to an `Array` using `#to_a`.
    #
    # @example
    #   Immutable::Vector["A", "B", "C"] - ["C", "D"]
    #   # => Immutable::Vector["A", "B"]
    #
    # @param other [Enumerable] The collection to concatenate onto this vector
    # @return [Vector]
    def -(other)
      other = other.to_a
      self.class.new((to_a - other).freeze)
    end
