...
# Modified to be wrapped under the TT module.
...
module FileUtils
  ...
  # 
  module Verbose
    include ::TT::FileUtils
    @fileutils_output  = $stderr
    @fileutils_label   = ''
    ::TT::FileUtils.collect_method(:verbose).each do |name|
    ...
    extend self
    class << self
      ::TT::FileUtils::METHODS.each do |m|
        public m
      end
    end
  end
  ...
  module NoWrite
    ...
    @fileutils_output  = $stderr
    @fileutils_label   = ''
    ::TT::FileUtils.collect_method(:noop).each do |name|
      module_eval(<<-EOS, __FILE__, __LINE__ + 1)
        def #{name}(*args)
